    ...
    for ShaRead<R, C, Challenge255<C>, D>
{
    fn read_point(&mut self) -> Result<C> {
        ...
        let point: C = Option::from(C::from_xy(x, y))
            .ok_or_else(|| Error::new(ErrorKind::Other, "invalid point encoding in proof"))?;
    ...
    fn read_scalar(&mut self) -> Result<C::Scalar> {
        ...
        let scalar: C::Scalar = Option::from(C::Scalar::from_repr(data)).ok_or_else(|| {
            Error::new(ErrorKind::Other, "invalid field element encoding in proof")
        })?;
    ...
    for ShaRead<R, C, Challenge255<C>, D>
{
    fn squeeze_challenge(&mut self) -> Challenge255<C> {
        ...
        let mut result: [u8; 32] = hasher.finalize().as_slice().try_into().unwrap();
