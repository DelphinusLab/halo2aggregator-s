...
pub struct VerifierParamsBuilder<
    'a,
    E: MultiMillerLoop,
    PCS: PolynomialCommitmentScheme<E::Scalar, CommitmentChunk = E::G1Affine>,
> {
    pub(crate) key: String,
    pub(crate) proof_index: usize,
    pub(crate) params: &'a ParamsVerifier<E>,
    pub(crate) vk: &'a HyperPlonkVerifierParam<E::Scalar, PCS>,
}
...
impl<
        ...
        PCS: PolynomialCommitmentScheme<E::Scalar, CommitmentChunk = E::G1Affine>,
    > VerifierParamsBuilder<'a, E, PCS>
{
    fn init_transcript(&self, proof_index: usize) -> (Vec<AstPointRc<C>>, Rc<AstTranscript<C>>) {
        let instance_commitments = (0..self.vk.num_instances.len())
            .into_iter()
    ...
    pub fn build(
        ...
        let n_advice = self.vk.num_witness_polys[0];
        let advice_commitments = transcript
            ...
            .collect::<Vec<_>>();
        let preprocess_comms = self
            .vk
            .preprocess_comms
            .iter()
            .map(|v| v.as_ref()[0])
            .collect::<Vec<C>>();
        let permutation_comms = self
            .vk
            .permutation_comms
            .iter()
            .map(|(_, v)| v.as_ref()[0])
        ...
        let lookup_m_commitments = transcript
            ...
            .into_iter()
            .enumerate()
            .map(|(i, x)| pcheckpoint!(format!("lookup commitment {} {}", self.proof_index, i), x))
            .collect::<Vec<AstPointRc<C>>>();

        let gamma = transcript.squeeze_challenge();

        let lookup_z_permu_z_commitments =
        ...
        // let gates = self.vk.expression.clone();
        // let dummy_comm = Pcs::Commitment::default();
        let fixed_commits = preprocess_comms
            .iter()
            .map(|p| pconst!(*p))
            .collect::<Vec<_>>();
        let permut_commits = permutation_comms
            .iter()
            .map(|p| pconst!(*p))
            .collect::<Vec<_>>();
        let comms = chain![
            ...
            &lookup_z_permu_z_commitments,
        ]
        .collect::<Vec<_>>();
        //TODO check num_instances[0]
        let querys = pcs_query(&self.vk.expression, self.vk.num_instances.len());
        // let evals = transcript.read_n_scalars(querys.len());
        ...
        //need correct sequence, keep all cur in head
        query_eval_cur.append(&mut query_eval_rot);

        //calc expressions on poly query evals, it should be equal to sumcheck_eval
        //sumcheck is firstly to compose all poly by expression to one virtual poly and then calc on challenges
        //expression eval is firstly to get all poly eval on challenges and then compose them by expression
        //this expression_eval just like the oracle to sumcheck last step and make sure consistent sumcheck poly.
        ...
        // let z = AstScalarRc(Rc::new(AstScalar::CheckPoint(format!("z.point"),z.0.clone())));
        // let point: Vec<AstScalarRc<C>> = vec![];
        ...
        let t = scalar!(z) * f_commit
            ...
            //embed equal-checking for (expression_eval,sumcheck_eval) to pairing checking
            //if expression_eval-sumcheck_eval==0, no impact to paring, if !=0, final pairing fail
            + scalar!(expression_eval-sumcheck_eval)
            + scalar!(x) * commit!(pi.clone());
        //[wx(quotient or pi),s_g2],[wg(pi+witness),-g2]
        let w_g = q_scalars
            .iter()
